alter table chronos.skills
  add column if not exists lifetime_adjustment_seconds bigint not null default 0;

create or replace function chronos.set_skill_lifetime_seconds(
  p_skill_id uuid,
...
declare
  ...
  v_target_seconds bigint := greatest(0, coalesce(p_lifetime_seconds, 0));
begin
  select exists (
    select 1
  ...
  select coalesce(sum(greatest(0, extract(epoch from se.ended_at - se.started_at)))::bigint, 0)
  into v_counted_seconds
  ...
  set lifetime_adjustment_seconds = v_target_seconds - coalesce(v_counted_seconds, 0)
  ...
  values (
    v_user_id,
    'set_skill_lifetime_seconds',
    'skill',
    v_skill.id,
    jsonb_build_object(
      'skill_slug', v_skill.slug,
      'target_seconds', v_target_seconds,
      'counted_session_seconds', coalesce(v_counted_seconds, 0),
      'lifetime_adjustment_seconds', v_skill.lifetime_adjustment_seconds
    )
  );
  ...
  return jsonb_build_object(
    ...
    'skill', jsonb_build_object(
      ...
      'slug', v_skill.slug,
      'name', v_skill.name,
      'lifetime_seconds', v_target_seconds,
      'lifetime_adjustment_seconds', v_skill.lifetime_adjustment_seconds
    )
  );
end;
...
begin
  ...
  rollups as (
    ...
    cross join bounds b
    left join counted_sessions se on se.skill_id = sk.id
    group by sk.id, sk.lifetime_adjustment_seconds
  ),
  active_session as (
    select
...
revoke all on function chronos.set_skill_lifetime_seconds(uuid, bigint) from public;
grant execute on function chronos.set_skill_lifetime_seconds(uuid, bigint) to authenticated, service_role;
